...
### 你是 macOS / Windows（Docker Desktop）
...
```mermaid
flowchart LR
    C[客户端 / SDK] --> A[clouisle-apiserver]
    A --> S[(SQLite / PostgreSQL)]
    A -->|帧协议 TCP 5201| G[guest agent]
    A --> V[Firecracker / docker-dev / 远程节点]
    A --> F[netns + nftables + DNS 代理]
```
